...
#define THUG_NOSHADOW	128// FREDZ
...
// END JOSEPH
void Thugkilledmessage (edict_t *self, edict_t *inflictor, edict_t *attacker)//FREDZ
{
	int			mod;
	char		*message;
	char		*message2;
...
/*			case MOD_BFG_BLAST:
		...
		{
			if (self->name)
				gi.bprintf (PRINT_MEDIUM,"%s(Thug) %s.\n", self->name, message);
			else
				gi.bprintf (PRINT_MEDIUM,"Thug %s.\n", message);
			self->enemy = NULL;
			return;
		}
		self->enemy = attacker;
...
/*			attacker->client->resp.killstreak++;
	...
	if (self->name)
		gi.bprintf (PRINT_MEDIUM,"%s(Thug) died.\n", self->name);
	else
		gi.bprintf (PRINT_MEDIUM,"Thug died.\n");
}//FREDZ end
// JOSEPH 26-FEB-99
void thug_die (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point, int mdx_part, int mdx_subobject)
{
	...
	{
		Thugkilledmessage (self, inflictor, attacker);
	}
...
//	self->cast_info.move_start_climb = &thug_move_st_clmb;
	...
	if (self->spawnflags & THUG_NOSHADOW)//FREDZ
