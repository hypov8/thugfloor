...
#define RUNT_NOSHADOW	128// FREDZ
...
void Runtkilledmessage (edict_t *self, edict_t *inflictor, edict_t *attacker)//FREDZ
{
	int			mod;
	char		*message;
	char		*message2;
...
/*			case MOD_BFG_BLAST:
		...
		{
			if (self->name)
				gi.bprintf (PRINT_MEDIUM,"%s(Runt) %s.\n", self->name, message);
			else
				gi.bprintf (PRINT_MEDIUM,"Runt %s.\n", message);
			self->enemy = NULL;
			return;
		}
		self->enemy = attacker;
...
/*			attacker->client->resp.killstreak++;
	...
	if (self->name)
		gi.bprintf (PRINT_MEDIUM,"%s(Runt) died.\n", self->name);
	else
		gi.bprintf (PRINT_MEDIUM,"Runt died.\n");
}//FREDZ end
// JOSEPH 19-MAR-99
void runt_die (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point, int mdx_part, int mdx_subobject)
{
	...
	{
		Runtkilledmessage (self, inflictor, attacker);
	}
...
/*	if (deathmatch->value)
	...
	if (Q_strcasecmp(self->name, "Sharky"))
	{
...
//	self->cast_info.move_start_climb = &runt_move_st_clmb;
	...
	if (self->spawnflags & RUNT_NOSHADOW)//FREDZ
