...
#define PUNK_FLASH_LIGHT    1//FREDZ missing
...
//		fire_flamethrower(self, start, aim, damage, 0, MOD_FLAMETHROWER);
        cast_fire_flamethrower(self, start, aim, damage, 0, MOD_FLAMETHROWER);
...
void Punkkilledmessage (edict_t *self, edict_t *inflictor, edict_t *attacker)//FREDZ
{
	int			mod;
	char		*message;
	char		*message2;
...
/*			case MOD_BFG_BLAST:
		...
		{
			if (self->name)
				gi.bprintf (PRINT_MEDIUM,"%s(Punk) %s.\n", self->name, message);
			else
				gi.bprintf (PRINT_MEDIUM,"Punk %s.\n", message);
			self->enemy = NULL;
			return;
		}
		self->enemy = attacker;
...
/*			attacker->client->resp.killstreak++;
	...
	if (self->name)
		gi.bprintf (PRINT_MEDIUM,"%s(Punk) died.\n", self->name);
	else
		gi.bprintf (PRINT_MEDIUM,"Punk died.\n");
}//FREDZ end
// JOSEPH 26-FEB-99
...
// regular death
	...
	{
		Punkkilledmessage (self, inflictor, attacker);
	}
