...
static void fire_lead (edict_t *self, vec3_t start, vec3_t aimdir, int damage, int kick, int te_impact, int hspread, int vspread, int mod)
{
	...
	if (!(tr.fraction < 1.0))
	{
		...
		else if (tr.surface->flags & SURF_WOOD)
		{
			...
			{
				case 0:
					gi.sound (self, CHAN_BULLET, gi.soundindex ("weapons/bullethit_wood1.wav"), 1, ATTN_NORM, 0); // MH: don't clobber voice channel
					break;
				case 1:
					gi.sound (self, CHAN_BULLET, gi.soundindex ("weapons/bullethit_wood2.wav"), 1, ATTN_NORM, 0); // MH: don't clobber voice channel
					break;
				case 2:
					gi.sound (self, CHAN_BULLET, gi.soundindex ("weapons/bullethit_wood3.wav"), 1, ATTN_NORM, 0); // MH: don't clobber voice channel
					break;
			}
		}
		// END JOSEPH
...
//											tr.ent, tr.endpos, tr.plane.normal );
	...
	if (hit) self->client->resp.acchit++; // MH: counting accuracy in bagman now too
}
// END JOSEPH
...
{
	G_DoTimeShiftFor(self); // MH: antilag stuff

	fire_lead (self, start, aimdir, damage, kick, TE_GUNSHOT, hspread, vspread, mod);

	G_UndoTimeShiftFor(self); // MH: undo antilag stuff
}
...
void rocket_touch (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (other->takedamage)
	{
		...
		if (!ent->inuse) return; // MH: rocket got destroyed by the target
		// MH: don't include rockets in accuracy stats
	...
	if (!((tr.surface) && (tr.surface->flags & SURF_SKY)))
	{
		if (tr.fraction < 1.0)
		{
			...
			else if ((mod != MOD_DOGBITE) && (mod != MOD_RATBITE) && (mod != MOD_FISHBITE))//FREDZ
			{
				...
				{
					gi.sound (self, CHAN_AUTO, gi.soundindex ("weapons/melee/pipehitcement.wav"), 1, ATTN_NORM, 0); // MH: don't clobber voice channel
				}
...
{
	fire_concussion (self, start, aimdir, 40, damage, 1, TE_GUNSHOT, 0, 0, MOD_RATBITE);
}
// END JOSEPH
/*
=================
...
================
*/
void fire_fish (edict_t *self, vec3_t start, vec3_t aimdir, int damage)
{
	fire_concussion (self, start, aimdir, 40, damage, 1, TE_GUNSHOT, 0, 0, MOD_FISHBITE);
}
/*
=================
fire_blackjack
