...
skipbail:
	...
	{
		if (self->acc)
			cast_fire_tommygun (self, start, aim, 4, 0, DEFAULT_BULLET_HSPREAD>>self->acc, DEFAULT_BULLET_VSPREAD>>self->acc, flash_number);
			//FREDZ it's tommygun
//			cast_fire_bullet (self, start, aim, 4, 0, DEFAULT_BULLET_HSPREAD>>self->acc, DEFAULT_BULLET_VSPREAD>>self->acc, flash_number);
		else
			cast_fire_tommygun (self, start, aim, 4, 0, DEFAULT_BULLET_HSPREAD, DEFAULT_BULLET_VSPREAD, flash_number);
		//FREDZ it's tommygun
//		cast_fire_bullet (self, start, aim, 4, 0, DEFAULT_BULLET_HSPREAD, DEFAULT_BULLET_VSPREAD, flash_number);
    ...
    else if (self->spawnflags & SHORTY_HMG)//FREDZ missing
	{
	//	fire_bullet (self, start, aim, 15, 50, DEFAULT_BULLET_HSPREAD>>self->acc, DEFAULT_BULLET_VSPREAD>>self->acc, MOD_BARMACHINEGUN);
        cast_fire_barmachinegun (self, start, aim, 15, 50, DEFAULT_BULLET_HSPREAD>>self->acc, DEFAULT_BULLET_VSPREAD>>self->acc, MOD_BARMACHINEGUN);//FREDZ
		{
...
//		fire_flamethrower(self, start, aim, damage, 0, MOD_FLAMETHROWER);
        cast_fire_flamethrower(self, start, aim, damage, 0, MOD_FLAMETHROWER);//FREDZ
...
void Shortykilledmessage (edict_t *self, edict_t *inflictor, edict_t *attacker)//FREDZ
{
	int			mod;
	char		*message;
	char		*message2;
...
/*			case MOD_BFG_BLAST:
		...
		{
			if (self->name)
				gi.bprintf (PRINT_MEDIUM,"%s(Shorty) %s.\n", self->name, message);
			else
				gi.bprintf (PRINT_MEDIUM,"Shorty %s.\n", message);
			self->enemy = NULL;
			return;
		}
		self->enemy = attacker;
...
/*			attacker->client->resp.killstreak++;
	...
	if (self->name)
		gi.bprintf (PRINT_MEDIUM,"%s(Shorty) died.\n", self->name);
	else
		gi.bprintf (PRINT_MEDIUM,"Shorty died.\n");
}//FREDZ end
// JOSEPH 26-FEB-99
...
// regular death
	...
	{
		Shortykilledmessage (self, inflictor, attacker);
	}
...
/*	if (deathmatch->value)
	...
	self->s.num_parts++;
/*
	switch (self->head)//FREDZ only head.mdx exist
	{
		case 1:
...
//self->spawnflags |= SHORTY_SHOTGUN;
	...
	//FREDZ new models never used
	if (self->count & 1) // cigar
	{
		self->s.num_parts++;
		self->s.model_parts[PART_CIGAR].modelindex = gi.modelindex("models/actors/shorty/cigar.mdx");
		for (i=0; i<MAX_MODELPART_OBJECTS; i++)
			self->s.model_parts[PART_CIGAR].baseskin = self->s.model_parts[PART_CIGAR].skinnum[i] = 0;
		gi.GetObjectBounds( "models/actors/shorty/cigar.mdx", &self->s.model_parts[PART_CIGAR] );
	}

	if (self->count & 2) // fedora hat
	{
		self->s.num_parts++;
		self->s.model_parts[PART_HAT].modelindex = gi.modelindex("models/actors/shorty/fedora.mdx");
		for (i=0; i<MAX_MODELPART_OBJECTS; i++)
			self->s.model_parts[PART_HAT].baseskin = self->s.model_parts[PART_HAT].skinnum[i] = 0;
		gi.GetObjectBounds( "models/actors/shorty/fedora.mdx", &self->s.model_parts[PART_HAT] );
	}
	else if (self->count & 4) // stetson hat
	{
		self->s.num_parts++;
		self->s.model_parts[PART_HAT].modelindex = gi.modelindex("models/actors/shorty/stetson.mdx");
		for (i=0; i<MAX_MODELPART_OBJECTS; i++)
			self->s.model_parts[PART_HAT].baseskin = self->s.model_parts[PART_HAT].skinnum[i] = 0;
		gi.GetObjectBounds( "models/actors/shorty/stetson.mdx", &self->s.model_parts[PART_HAT] );
	}
	else if (self->count & 8) // cap (hat)
	{
		self->s.num_parts++;
		self->s.model_parts[PART_HAT].modelindex = gi.modelindex("models/actors/shorty/cap.mdx");
		for (i=0; i<MAX_MODELPART_OBJECTS; i++)
			self->s.model_parts[PART_HAT].baseskin = self->s.model_parts[PART_HAT].skinnum[i] = 0;
		gi.GetObjectBounds( "models/actors/shorty/cap.mdx", &self->s.model_parts[PART_HAT] );
	}//FREDZ new models never used end
...
// 	self->cast_info.move_crstand = &shorty_move_crouch_stand;
	...
	// RAFAEL
	if (self->spawnflags & SHORTY_FLASH_LIGHT)
