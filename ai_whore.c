...
#define WHORE_FLASH_LIGHT    1//FREDZ missing
...
void Whorekilledmessage (edict_t *self, edict_t *inflictor, edict_t *attacker)//FREDZ
{
	int			mod;
	char		*message;
	char		*message2;
...
/*			case MOD_BFG_BLAST:
		...
		{
			if (self->name)
				gi.bprintf (PRINT_MEDIUM,"%s(Whore) %s.\n", self->name, message);
			else
				gi.bprintf (PRINT_MEDIUM,"Whore %s.\n", message);
			self->enemy = NULL;
			return;
		}
		self->enemy = attacker;
...
/*			attacker->client->resp.killstreak++;
	...
	if (self->name)
		gi.bprintf (PRINT_MEDIUM,"%s(Whore) died.\n", self->name);
	else
		gi.bprintf (PRINT_MEDIUM,"Whore died.\n");
}//FREDZ end
// JOSEPH 26-FEB-99
...
// regular death
	...
	{
		Whorekilledmessage (self, inflictor, attacker);
	}
...
//self->spawnflags |= WHORE_SHOTGUN;
	...
	if (self->count & 1) //FREDZ new model never used cigar
	{
		self->s.num_parts++;
		self->s.model_parts[PART_CIGAR].modelindex = gi.modelindex("models/actors/whore/cigar.mdx");
		for (i=0; i<MAX_MODELPART_OBJECTS; i++)
			self->s.model_parts[PART_CIGAR].baseskin = self->s.model_parts[PART_CIGAR].skinnum[i] = 0;
		gi.GetObjectBounds( "models/actors/whore/cigar.mdx", &self->s.model_parts[PART_CIGAR] );
	}
...
// 	self->cast_info.move_crstand = &whore_move_crouch_stand;
	...
	// RAFAEL
	if (self->spawnflags & WHORE_FLASH_LIGHT)
