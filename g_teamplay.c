...
//			UPDATESCORE
			...
			other->client->resp.stole+=amount; // MH: changed from acchit
		}
...
//	InitClientPersistant (self->client);
	...
	if ((level.modeset != STARTINGMATCH) && (level.modeset != STARTINGPUB))
	{
		if (teamplay->value)//FREDZ
			...
			gi.bprintf( PRINT_HIGH, "%s joined\n", self->client->pers.netname);
...
//		self->client->resp.enterframe = level.framenum; // MH: unnecessary
