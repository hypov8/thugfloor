...
// MH: kpded2 options that can be enabled via the "g_features" cvar
...
#define FREEFORALL			0
//#define FINALCOUNT			1
//#define MATCHSETUP			2 //Check if server need to be reseted when no players
//#define MATCH				3
//#define MATCHEND			4
//#define STARTINGMATCH		5
#define TEAMPLAY			6//Whrong name if there is none teamplay used for checking CheckDMRules
#define STARTINGPUB			7
#define	ENDMATCHVOTING		8

//FREDZ disable and replaced names
// MH: MM2 code uses different names
//#define PREGAME				FREEFORALL
//#define PUBLICSPAWN			STARTINGPUB
//#define PUBLIC				TEAMPLAY
//#define ENDGAMEVOTE			ENDMATCHVOTING
//#define MATCHCOUNT			FINALCOUNT
...
#define	GAMEVERSION	"Thug Floor v0.29"
...
typedef enum
{
	...
	AMMO_ROCKETS,
	AMMO_GRENADES,
	AMMO_GAS,//FREDZ was AMMO_CELLS
	AMMO_308CAL//FREDZ was AMMO_SLUGS
//	AMMO_MAGSLUG,//FREDZ Disable or should be acid or drum
//	AMMO_TRAP//FREDZ Disable or should be acid or drum
} ammo_t;
...
typedef enum
{
	...
	CLIP_SHOTGUN,
	CLIP_TOMMYGUN,
	CLIP_308CAL,//FREDZ was CLIP_SLUGS
	CLIP_GRENADES,
	CLIP_ROCKETS,
	CLIP_FLAMEGUN
} clip_t;
...
// snap, struct to store raincloud stuff
typedef struct
{
	int			firetype;
	int			fxdensity;
...
// FREDZ, struct to store snowcloud stuff
typedef struct
{
	int			fxdensity;
	vec3_t		neworigin;
...
#define MOD_RATBITE				50
#define MOD_FISHBITE			51
#define MOD_HURT_ELECTRIC		52
#define MOD_HURT_FLAMES			53
...
extern	cvar_t	*no_zoom; // MH: FOV zooming option
...
extern	cvar_t	*antilag; // MH: antilag option

extern	int		starttime; // MH: server start time (replaces old uptime counters)

extern	qboolean	kpded2; // MH: the server is kpded2
...
void cast_fire_flamethrower (edict_t *self, vec3_t start, vec3_t forward, int damage, int kick, int flashtype);//FREDZ flamethrower missing
...
void GibGlassEntity (edict_t *self, edict_t *inflictor, float damage);//FREDZ
...
typedef struct
{
	...
	int			max_rockets;
	int			max_grenades;
	int			max_gas;//FREDZ was max_cells
	int			max_308cal;//FREDZ was max_slugs
	// RAFAEL
//	int			max_magslug;//FREDZ Disable or should be acid or drum
//	int			max_trap;//FREDZ Disable or should be acid or drum
	...
	int         fakeThief; // MH: moved here from below

// MH: everything above is reset at respawn, everything below isn't ("version" must be 1st)

	int			version; // MH: moved here from above
	int			patched; // MH: patch version

	int			team; // MH: moved here from above
...
// Papa 10.6.99 Admin stuff
	...
	char		ip[32];
	char		country[32];	// MH: country of ip
	char		rconx[32];
...
// Papa 10.6.99 Admin stuff
	...
	int			time; // MH: added to count playing time
...
struct gclient_s
{
	...
	// MH: also known to kpded2 server (enabled via g_features)
	...
	int			damage_fire;		// MH: fire damage
...
//  Papa
	...
	int			move_frame;
	char		skins[32];
	char		skinteam1[32];//FREDZ
	char		skinteam2[32];//FREDZ
	int			anonwarn;
	int			kickdelay;
	char		*kickmess;
...
extern int wait_for_players; // MH: wait for players
...
#define SCORENAME "scores" // MH: trimmed
#define TIMENAME "time" // MH: trimmed
