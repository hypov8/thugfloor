...
//    no_spec = gi.cvar ("no_spec", "0", 0); // MH: removed from serverinfo to avoid exceeding limit
    no_shadows = gi.cvar ("no_shadows", "0", 0); // MH: removed from serverinfo to avoid exceeding limit
	no_zoom = gi.cvar ("no_zoom", "0", 0); // MH: FOV zooming option
...
//	marines = gi.cvar ("marines", "0", CVAR_ARCHIVE);
	...
	antilag = gi.cvar("antilag", "1", CVAR_SERVERINFO);	// MH: antilag option

	{ // MH: server start time (replaces old uptime counters)
		char buf[20];
...
/*	gi.cvar(TEAMNAME, "", CVAR_SERVERINFO);
    ...
    kick_flamehack = gi.cvar ("kick_flamehack","1",CVAR_SERVERINFO); // MH: enabled by default
    anti_spawncamp = gi.cvar ("anti_spawncamp","1",0); // MH: removed from serverinfo to avoid exceeding limit
    idle_client	= gi.cvar("idle_client", "120", 0); // MH: changed from 60
...
// Ridah, new cvar's
	...
	maxwaves		= gi.cvar("maxwaves","2", CVAR_LATCH|CVAR_SERVERINFO); //hypov8 todo: add to comp.ini?
...
/*
	...
	cashlimit = gi.cvar ("cashlimit", "0", teamplay->value == 1 ? CVAR_SERVERINFO : 0); // MH: only add "cashlimit" to serverinfo for bagman
...
// Papa 10.6.99
	...
	if (i != OK)
	{
		...
		gi.dprintf("Processed thugfloor.ini file!\n");
	...
	// MH: disable voting if there are no maps
	if (!num_custom_maps)
		allow_map_voting = false;
    ...
    lockmouse[i]=0;
	for (i=0;i<7;i++) ver_check[i]='a'+(rand()%26); // FREDZ check mod version
	ver_check[i]='\0';

	// MH: enable kpded2 features
	if (kpded2)
		gi.cvar_forceset("g_features", va("%d", GMF_CLIENTPOV | GMF_WANT_ALL_DISCONNECTS | GMF_WANT_COUNTRY));
}
...
// snap, disabled this because i redid the raincloud code
