...
//void SP_cast_buma (edict_t *self);//FREDZ files does not excist for cast_buma disable
//void SP_cast_bumb (edict_t *self);//FREDZ files does not excist for cast_bumb disable
...
//	{"cast_buma", SP_cast_buma},//FREDZ files does not excist for cast_buma disable
//	{"cast_bumb", SP_cast_bumb},//FREDZ files does not excist for cast_bumb disable
...
// done.
	...
	// MH: fix any neutral spawn points close to a safe
...
// MH: added weapon timer to team DM
// weapon timer
"if 19 "
"xr -75 "
"yt 130 "
...
//	Teamplay_InitTeamplay();
	...
	// MH: set CS_CDTRACK first so that CS_NAME can overwrite it if needed for long map names
...
//		gi.configstring (CS_STATUSBAR, dm_statusbar);
        gi.configstring (CS_STATUSBAR, thugfloor_statusbar);//FREDZ
	}
	else
	{
...
//
	...
	if(allow_map_voting && custom_map_filename[0]!=0)
	{
		...
		while ((!found) && (i < (num_custom_maps - 1) ))
		{
			...
			{
				vote_set[0] = i; // MH: exclude current map
				vote_set[1] = i+1;
				found = true;
			}
			i++;
		...
		if (!found)
		{
			vote_set[0] = -1; // MH: no current map to exclude
			vote_set[1] = 0;
		}
		...
		else
		{
			...
			// MH: all other maps are picked at random
			for (i=2; i<9; i++)
			{
				unique = false;
				while (!unique)
