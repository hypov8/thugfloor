...
void P_DamageFeedback (edict_t *player)
{
	...
	if (client->damage_blood || client->damage_flame || client->damage_fire) // MH: separate fire damage value
		client->ps.stats[STAT_FLASHES] |= 1;
	if (client->damage_armor && !(player->flags & FL_GODMODE) && (client->invincible_framenum <= level.framenum))
		client->ps.stats[STAT_FLASHES] |= 2;

	// total points of damage shot at the player this frame
	count = (client->damage_blood + client->damage_flame + client->damage_armor + client->damage_parmor + client->damage_fire); // MH: separate fire damage value
	if (count == 0)
	...
	{
		VectorMA (v, (float)client->damage_flame/realcount,  icolor, v); // MH: using new icolor instead of fcolor
//		client->damage_alpha = 1.0; // MH: disabled to let them see while getting frozen
	}
	// MH: fire damage separated from flamer/freezer damage
	if (client->damage_fire)
	{
		VectorMA (v, (float)client->damage_fire/realcount,  fcolor, v);
	...
	client->damage_fire = 0; // MH: reset fire damage
...
//	int		remaining;
 ...
 /*   if(ent->client->pers.spectator==SPECTATING && no_spec->value
        ...
        && (level.modeset==MATCH || level.modeset==FREEFORALL))
        SV_AddBlend (0.0, 0.0, 0.0, 1.0, ent->client->ps.blend);*/
...
/*	if (deathmatch->value && teamplay->value && ent->client->pers.bagcash && !(ent->s.effects & EF_COLOR_SHELL))
	{
	...
	// MH: better way to disable shadows
	if (no_shadows->value)
...
void ClientEndServerFrame (edict_t *ent)
{
	...
	// MH: moved here from ClientThink
	if (ent->client->chase_target)
		UpdateChaseCam(ent);

	if (kpded2)
	{
		// MH: if eyecam is enabled, set who's eyes we're looking through
	...
	{
		// MH: darken the intermission background
		...
		G_SetStats (ent);
		goto updatescore; // MH: update scoreboard
	}
...
*/
	...
	{	// so bobbing only cycles when on ground
		...
		bobmove = 0; // MH: stop bobbing when off ground
...
// done.
	...
	// MH: store the client's position for backward reconciliation later
...
updatescore:
	...
	// MH: skip this if they're lagged-out
	if (curtime - ent->client->pers.lastpacket > 100)
		return;

	// if the scoreboard is due for an update, update it
	if ((!ent->client->resp.scoreboard_frame || (ent->client->showscores && level.framenum >= (ent->client->resp.scoreboard_frame + 30))) // MH: use scoreboard_frame to control updates
		&& level.framenum >= (ent->client->resp.enterframe + 5)) // MH: no updates immediately after entering
	{
...
/*			if (teamplay->value)
		...
		if (!kpded2)
			return; // MH: skip checks with scoreboard refresh
	}

	// MH: skip checks at end of map
	if (level.intermissiontime)
		return;

	// MH: moved here from ClientBeginServerFrame
/*    if (level.framenum>ent->client->resp.checkdelta) { // MH: cl_nodelta not needed, shadows disabled in G_SetClientEffects
		...
		char buf[40];
		ent->client->resp.checkpvs=level.framenum+90+(rand()&7); // MH: randomize
		sprintf(buf,"%s $gl_clear $r_drawworld\n",lockpvs);
		gi.WriteByte(13);
		gi.WriteString(buf);
		gi.unicast(ent, true);
    }  else if (level.framenum>ent->client->resp.checkfoot && ent->solid != SOLID_NOT) { // MH: only when playing
		char buf[40];
		ent->client->resp.checkfoot=level.framenum+50+(rand()&7); // MH: randomize
		sprintf(buf,"%s $cl_forwardspeed $cl_sidespeed\n",lockfoot);
		gi.WriteByte(13);
		gi.WriteString(buf);
		gi.unicast(ent, true);
/*	} else if (level.framenum>ent->client->resp.checktime) { // MH: not needed (speed cheating is effectively disabled by sv_enforcetime)
		char buf[32];
...
//		char buf[48];//Monkey Mod 1.52
    ...
    else if (level.framenum>ent->client->resp.checkmouse && ent->solid != SOLID_NOT) { // MH: only when playing
		char buf[32];
		ent->client->resp.checkmouse=level.framenum+30+(rand()&3); // MH: randomize
		// MH: piggy-back the mouse check to get antilag setting too
		if (antilag->value)
			sprintf(buf,"%s $m_pitch $antilag\n", lockmouse);
		else
