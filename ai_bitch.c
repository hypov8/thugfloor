...
#define BITCH_NOSHADOW	128// FREDZ whrong name NOWEAPON changed to NOSHADOW
...
// END JOSEPH
void Bitchkilledmessage (edict_t *self, edict_t *inflictor, edict_t *attacker)//FREDZ
{
	int			mod;
	char		*message;
	char		*message2;
...
/*		case MOD_TARGET_BLASTER:
		...
		case MOD_HURT_ELECTRIC://FREDZ
			...
			break;
		case MOD_HURT_FLAMES://FREDZ extra check
...
/*			case MOD_BFG_BLAST:
		...
		{
			if (self->name)
				gi.bprintf (PRINT_MEDIUM,"%s(Bitch) %s.\n", self->name, message);
			else
				gi.bprintf (PRINT_MEDIUM,"Bitch %s.\n", message);
			self->enemy = NULL;
			return;
		}
		self->enemy = attacker;
...
/*			attacker->client->resp.killstreak++;
	...
	if (self->name)
		gi.bprintf (PRINT_MEDIUM,"%s(Bitch) died.\n", self->name);
	else
		gi.bprintf (PRINT_MEDIUM,"Bitch died.\n");
}//FREDZ end
// JOSEPH 11-JUN-99-B
...
//	self->s.modelindex2 = 0;
	...
	{
		Bitchkilledmessage (self, inflictor, attacker);
	}
...
//			self->s.model_parts[PART_HEAD].modelindex = gi.modelindex("models/actors/bitch/bald_head.mdx");//FREDZ Model does not exist
	...
	*/
	...
	if (self->spawnflags & BITCH_NOSHADOW)//FREDZ 128 to BITCH_NOSHADOW
