...
#define THUG_NOSHADOW	128// FREDZ also same as THUG_NO_SIT

#define THUG_NO_SIT		128

#include "ai_thug_sit_tables.h"
...
// END JOSEPH
void ThugSitkilledmessage (edict_t *self, edict_t *inflictor, edict_t *attacker)//FREDZ
{
	int			mod;
	char		*message;
	char		*message2;
...
/*			case MOD_BFG_BLAST:
		...
		{
			if (self->name)
				gi.bprintf (PRINT_MEDIUM,"%s(Thug sit) %s.\n", self->name, message);
			else
				gi.bprintf (PRINT_MEDIUM,"Thug sit %s.\n", message);
			self->enemy = NULL;
			return;
		}
		self->enemy = attacker;
		...
		if (message)
		{
			...
			TF_giveCashOnKill(BOT_THUG_SIT, self);//FREDZ give cash
...
/*			attacker->client->resp.killstreak++;
	...
	if (self->name)
		gi.bprintf (PRINT_MEDIUM,"%s(Thug sit) died.\n", self->name);
	else
		gi.bprintf (PRINT_MEDIUM,"Thug sit died.\n");
}//FREDZ end
// JOSEPH 20-NOV-98
void thug2_die (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point, int mdx_part, int mdx_subobject)
{
	...
	{
		ThugSitkilledmessage (self, inflictor, attacker);
	}
...
/*	if (deathmatch->value)
	...
	switch (self->head)
	{
		case 1:
			...
			break;
		case 2:
//			self->s.model_parts[PART_HEAD].modelindex = gi.modelindex("models/actors/thug_sit/ski_head.mdx");//FREDZ Model does not exist
...
//	self->cast_info.move_start_climb = &thug2_move_st_clmb;
	...
	if (self->spawnflags & THUG_NOSHADOW)
