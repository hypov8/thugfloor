...
//#define	MAX_FLAMEGUN_ROUNDS			50//FREDZ not used for reload
...
void ChangeClipIndex (edict_t *ent)
{
	...
	else if (!strcmp(ent->client->pers.weapon->pickup_name, "Pistol Silencer"))
		ent->client->clip_index = CLIP_PISTOL;
	...
	// JOSEPH 16-APR-99
		...
		ent->client->clip_index = CLIP_308CAL;
	// END JOSEPH
}
...
int QweryClipIndex (gitem_t *item)
{
	...
	if (!strcmp(item->pickup_name, "Pipe"))
		...
		return CLIP_PISTOL;
	else if (!strcmp(item->pickup_name, "Pistol Silencer"))
		return CLIP_PISTOL;
...
void ChangeWeapon (edict_t *ent)
{
...

	{
		...
		if (!strcmp (ent->client->pers.weapon->pickup_name , "Pistol Silencer"))
		{
			if (ent->client->pers.inventory[ITEM_INDEX(FindItem("Pistol Magnum Mod"))])
		...
		else if (!strcmp(ent->client->pers.weapon->pickup_name, "Bazooka"))
		{
			...
			*/
			...
			//FREDZ  missing mod that is default on shoot multiple rockets
			/*
...
void NoAmmoWeaponChange (edict_t *ent)
{
	...
	if ( (ent->client->pers.inventory[ITEM_INDEX(FindItem("grenades"))] || ent->client->pers.weapon_clip[CLIP_GRENADES]) // MH: fixed to also check current clip
	...
	// JOSEPH 16-APR-99
	if ( (ent->client->pers.inventory[ITEM_INDEX(FindItem("308cal"))] || ent->client->pers.weapon_clip[CLIP_308CAL]) // MH: fixed to also check current clip
    ...
    if ( (ent->client->pers.inventory[ITEM_INDEX(FindItem("rockets"))] || ent->client->pers.weapon_clip[CLIP_ROCKETS]) // MH: fixed to also check current clip
		&&  ent->client->pers.inventory[ITEM_INDEX(FindItem("bazooka"))] )
	{
		ent->client->newweapon = FindItem ("Bazooka");
		return;
	}
    //FREDZ rockets before flamethrower
	...
	if ( (ent->client->pers.inventory[ITEM_INDEX(FindItem("bullets"))] || ent->client->pers.weapon_clip[CLIP_TOMMYGUN]) // MH: fixed to also check current clip
	...
	if ( (ent->client->pers.inventory[ITEM_INDEX(FindItem("shells"))] || ent->client->pers.weapon_clip[CLIP_SHOTGUN]) // MH: fixed to also check current clip
	...
	if ( (ent->client->pers.inventory[ITEM_INDEX(FindItem("bullets"))] || ent->client->pers.weapon_clip[CLIP_PISTOL]) // MH: fixed to also check current clip
...
// RAFAEL 01-11-99
	...
	{
		if (ent->client->pers.weapon == FindItem("Pistol Silencer"))
		{
...
*/
	...
	if (ent->client->weaponstate == WEAPON_READY)
	{
		if (((ent->client->latched_buttons|ent->client->buttons) & BUTTON_ATTACK) && (!level.buyzone))//FREDZ seems to work, not sure if it works for all weapons yet
		{
			...
			else if (ent->client->clip_index == CLIP_308CAL && ent->client->pers.weapon_clip[ent->client->clip_index] >=1)
			{
				ent->client->ps.gunframe = FRAME_FIRE_FIRST;
				ent->client->weaponstate = WEAPON_FIRING;
			}
			...
			else
			{
				...
				{
					gi.sound(ent, CHAN_AUTO, gi.soundindex("weapons/noammo.wav"), 1, ATTN_NORM, 0); // MH: don't clobber voice channel
...
void Weapon_Pistol_Fire (edict_t *ent)
{
	...
	{
		if (ent->client->pers.inventory[ITEM_INDEX(FindItem("Pistol RoF Mod"))])
		{
			if (ent->client->pers.inventory[ITEM_INDEX(FindItem("Pistol Magnum Mod"))])
			{
				damage *= 2;
	...
	else if (ent->client->ps.gunframe == 10)
	{
		if (!ent->client->pers.weapon_clip[ent->client->clip_index])
		{
			...
			ent->client->ps.gunframe++;
			if (ent->client->pers.inventory[ITEM_INDEX(FindItem("Pistol Reload Mod"))])
				;//gi.dprintf ("need fast reload sound\n");
			else
				gi.sound(ent, CHAN_AUTO, gi.soundindex("weapons/pistol/clip_out.wav"), 1, ATTN_NORM, 0); // MH: don't clobber voice channel
...
void Weapon_Pistol (edict_t *ent)
{
	...
	if (ent->client->ps.gunframe == 1)
		gi.sound(ent, CHAN_AUTO, gi.soundindex("weapons/pistol/holster.wav"), 1, ATTN_NORM, 0); // MH: don't clobber voice channel

	if (ent->client->ps.gunframe == 11)
		gi.sound(ent, CHAN_AUTO, gi.soundindex("weapons/pistol/clip_out.wav"), 1, ATTN_NORM, 0); // MH: don't clobber voice channel
/* // MH: not needed with new sounds
	if (ent->client->ps.gunframe == 18)
		gi.sound(ent, CHAN_AUTO, gi.soundindex("weapons/pistol/clip_in.wav"), 1, ATTN_NORM, 0); // MH: don't clobber voice channel
*/
...
// gi.dprintf ("silencer shots left: %d\n", ent->client->pers.silencer_shots);
	...
	if (ent->client->ps.gunframe == 1)
		gi.sound(ent, CHAN_AUTO, gi.soundindex("weapons/pistol/silencerattatch.wav"), 1, ATTN_NORM, 0); // MH: don't clobber voice channel
	...
	else if (ent->client->reload_weapon)
	{
		if (ent->client->ps.gunframe >= 30 && ent->client->ps.gunframe <= 40)
		{
			if (ent->client->pers.weapon_clip[ent->client->clip_index] < MAX_TOMMYGUN_ROUNDS)
			{
				...
				ent->client->pers.weapon_clip[ent->client->clip_index] = rounds;
				ent->client->pers.inventory[ent->client->ammo_index] -= rounds;
				gi.sound(ent, CHAN_AUTO, gi.soundindex("weapons/machinegun/machgcock.wav"), 1, ATTN_NORM, 0); // MH: don't clobber voice channel
			}
...
//	vec3_t		offset;
	...
	//FREDZ Flamethrower is too powerful in real-mode
	if (dm_realmode->value && rand()%2)
		damage = 1;
	...
	else if (ent->client->reload_weapon)
	{
		if (ent->client->ps.gunframe >= 43 && ent->client->ps.gunframe <= 53)
		{
			if (ent->client->pers.weapon_clip[ent->client->clip_index] < MAX_BARGUN_ROUNDS)
			{
				...
				ent->client->pers.weapon_clip[ent->client->clip_index] = rounds;
				ent->client->pers.inventory[ent->client->ammo_index] -= rounds;
				gi.sound(ent, CHAN_AUTO, gi.soundindex("weapons/hmg/hmgcock.wav"), 1, ATTN_NORM, 0); // MH: don't clobber voice channel
			}
...
void Weapon_GrenadeLauncher (edict_t *ent)
{
	...
	else if (ent->client->reload_weapon)
	{
		...
		{
			if (ent->client->pers.weapon_clip[ent->client->clip_index] < MAX_GRENADELAUNCHER_ROUNDS)
			{
				ent->client->ps.gunframe = 17;
				ent->client->weaponstate = WEAPON_RELOADING;
				...
				if (ent->client->pers.inventory[ent->client->ammo_index] < MAX_GRENADELAUNCHER_ROUNDS)
				{
					rounds = ent->client->pers.inventory[ent->client->ammo_index];
				}
				else
					rounds = MAX_GRENADELAUNCHER_ROUNDS;

				ent->client->pers.weapon_clip[ent->client->clip_index] = rounds;
				ent->client->pers.inventory[ent->client->ammo_index] -= rounds;
				gi.sound(ent, CHAN_AUTO, gi.soundindex("weapons/grenade_launcher/reload.wav"), 1, ATTN_NORM, 0); // MH: don't clobber voice channel
			}
...
void Weapon_RocketLauncher (edict_t *ent)
{
	...
	else if (ent->client->reload_weapon)
	{
		if (ent->client->ps.gunframe >= 30 && ent->client->ps.gunframe <= 41)
		{
				...
				if (ent->client->pers.weapon_clip[ent->client->clip_index] < MAX_ROCKETLAUNCHER_ROUNDS)
				{
					ent->client->ps.gunframe = 14;
					ent->client->weaponstate = WEAPON_RELOADING;
					...
					if (ent->client->pers.inventory[ent->client->ammo_index] < MAX_ROCKETLAUNCHER_ROUNDS)
					{
						rounds = ent->client->pers.inventory[ent->client->ammo_index];
					}
					else
						rounds = MAX_ROCKETLAUNCHER_ROUNDS;

					ent->client->pers.weapon_clip[ent->client->clip_index] = rounds;
					ent->client->pers.inventory[ent->client->ammo_index] -= rounds;
					gi.sound(ent, CHAN_AUTO, gi.soundindex("weapons/rocket_launcher/reload.wav"), 1, ATTN_NORM, 0); // MH: don't clobber voice channel
				}
