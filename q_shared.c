	...
	while (1)
	{
		...
		*o = 0;
		...
		if (!strcmp (key, pkey) )
		{
			...
			memmove (start, s, strlen(s) + 1);	//MH remove this part
			return;
		}
...
void Info_SetValueForKey (char *s, char *key, char *value)
{
	...
	if (strlen(newi) + strlen(s) > MAX_INFO_STRING-1) // MH: fix
	{
...
// MH: time retrieval stuff
#ifdef _WIN32
#include <time.h>//FREDZ no windows.h in other compilers
#else
#include <sys/time.h>
#endif
...
//		base = timeGetTime() & 0xffff0000; //FREDZ Uses horrible winmm.lib
        base = clock() & 0xffff0000;
		initialized = true;
	}
//	curtime = timeGetTime() - base;//FREDZ Uses horrible winmm.lib
    curtime = clock() - base;
#else
	struct timeval tp;
