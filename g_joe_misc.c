...
void crate_stuff (edict_t *self)
{
	...
	// MH: limit the debris in deathmatch to avoid overflows
...
int check_rat_hit_player(edict_t *ent)
{
	...
	if (tr.fraction < 1)
	{
		if ((tr.ent) && (tr.ent->client))
		{
			...
			//T_Damage (tr.ent, tr.ent, tr.ent, vec3_origin, tr.ent->s.origin, vec3_origin, ent->dmg, ent->dmg, 0, MOD_RATBITE);
...
void rat_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	T_Damage (other, self, self, vec3_origin, other->s.origin, vec3_origin, self->dmg, self->dmg, 0, MOD_RATBITE);
}
...
int check_fish_hit_player(edict_t *ent)
{
	...
	if (tr.fraction < 1)
	{
		if ((tr.ent) && (tr.ent->client))
		{
			...
			//T_Damage (tr.ent, tr.ent, tr.ent, vec3_origin, tr.ent->s.origin, vec3_origin, ent->dmg, ent->dmg, 0, MOD_FISHBITE);
