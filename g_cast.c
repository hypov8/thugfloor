...
//FREDZ q2 stuff fucks up alot in kp and not using
/*
...
{
/*	if (deathmatch->value)//FREDZ thugfloor
	{
		G_FreeEdict (self);
		return false;
	}*/
...
// Ridah, try to add them to the character list
	if (!(self->spawnflags & SPAWNFLAG_TRIGGERED_START) && !AddCharacterToGame(self))//FREDZ
	{
		G_FreeEdict (self);
		return false;
	}
// Ridah, done.
	...
	if (!deathmatch->value)//FREDZ maker everything shootble
		EP_SpawnFlags (self);
...
//self->flags |= FL_GODMODE;
	...
	if ((!strstr (level.mapname, "kpcut")) && (!strstr (level.mapname, "idiot")) && (!strstr (level.mapname, "end")))//FREDZ
...
// Ridah, 5-7-99, prevent guy at intro appearing upright after reloading SR1
			if (!(self->spawnflags & SPAWNFLAG_TRIGGERED_START) && (!self->name || (strcmp(self->name, "IntroGuy") != 0)))//FREDZ
...
{
	if (!(self->spawnflags & SPAWNFLAG_TRIGGERED_START) && level.time < 1)//FREDZ
	{
		M_droptofloor (self);
	...
	if ((self->cast_group == 1 && !self->name) && (developer->value))//FREDZ now only in developer otherwise maybe remove
	{
		gi.dprintf( "\n** WARNING: friendly %s without a NAME.\n  All friendly characters MUST have a name.\n\n", self->classname );
	}

	if (self->spawnflags & SPAWNFLAG_TRIGGERED_START)//FREDZ
