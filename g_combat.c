...
//	if (teamplay->value && targ && attacker && attacker->client && targ->client && (targ->client->pers.team) && (targ->client->pers.team == attacker->client->pers.team))//FREDZ need fix
//		return true;
//	else
		return false;
}*/
...
void T_Damage (edict_t *targ, edict_t *inflictor, edict_t *attacker, vec3_t dir, vec3_t point, vec3_t normal, int damage, int knockback, int dflags, int mod)
{
	...
	int			te_sparks;
	float		dmg;
	vec3_t		sparkspoint;	// MH: position of blood drops
	  ...
	  //  cprintf (targ, PRINT_HIGH, "1 brace\n");
        if ((OnSameTeam (targ, attacker)) && (!targ->client->pers.friendly_vulnerable))
		{
           // cprintf (targ, PRINT_HIGH, "2 brace\n");
			...
			{
				if ((mod == MOD_BLACKJACK) || (mod == MOD_CROWBAR))//FREDZ give health on team members
				{
					dmg = 0;
					if (targ->health<targ->client->pers.max_health)
						targ->health+=2;
...
//						gi.centerprintf (attacker, "(%s)%s has %i health!\n",team_names[targ->client->pers.team], targ->client->pers.netname, targ->health);
	...
	// MH: fix to make the blood come from the player (not the wall)
...
//		dmg *= 2;
	...
	// MH: no knockback if movement is disabled by a trigger_push
...
// figure momentum add
	...
	// END JOSEPH
	{
		if ((knockback) && (targ->movetype != MOVETYPE_NONE) && (targ->movetype != MOVETYPE_BOUNCE) && (targ->movetype != MOVETYPE_PUSH) && (targ->movetype != MOVETYPE_STOP))
		{
			...
			{
				VectorScale (dir, 500.0 * (float)knockback / mass, kvel);
				// MH: prevent hurting teammates by shotgunning them into the ground
	...
	if (targ->client && dmg && (!(dflags & DAMAGE_NO_ARMOR)))
	{
		...
		// MH: take armour according to height of hit
		...
		// MH: platforms and vertical doors hit head
      ...
      //  cprintf (targ, PRINT_HIGH, "9 brace\n");
	    ...
	    if ((item) && (targ->client->pers.inventory[ITEM_INDEX(item)]))
		{
          ...
          //  cprintf (targ, PRINT_HIGH, "10 brace\n");
			...
			if (mod == MOD_FLAMETHROWER) // MH: new MOD for fire
            {
	...
	if ( (targ->flags & FL_GODMODE) && !(dflags & DAMAGE_NO_PROTECTION) )
	{
       // cprintf (targ, PRINT_HIGH, "14 brace\n");
		...
		take = 0;
		save = dmg;
		SpawnDamage (targ, te_sparks, sparkspoint, normal, save); // MH: using corrected position
	}
		...
		else
		{
         //   cprintf (targ, PRINT_HIGH, "19 brace\n");
			...
			if ((targ->svflags & SVF_MONSTER) || (client))
				SpawnDamage (targ, TE_BLOOD, sparkspoint, normal, take); // MH: using corrected position
			else
...
//			if (!targ->deadflag)
				Killed (targ, inflictor, attacker, take, sparkspoint, 0, 0); // MH: using corrected position
...
//		else if (mod == MOD_FIRE) // MH: new MOD for fire
//			client->damage_fire += take*4;
		else
        {
			...
			if ((targ->svflags & SVF_MONSTER) || (client))
				if (hasarmor)
				{
...

					{
						...
						if (rval > 66)
							gi.sound(targ, CHAN_BULLET, gi.soundindex("weapons/bullethit_armor1.wav"), 1, ATTN_NORM, 0); // MH: don't clobber voice channel
						else if (rval > 33)
							gi.sound(targ, CHAN_BULLET, gi.soundindex("weapons/bullethit_armor2.wav"), 1, ATTN_NORM, 0); // MH: don't clobber voice channel
						else
							gi.sound(targ, CHAN_BULLET, gi.soundindex("weapons/bullethit_armor3.wav"), 1, ATTN_NORM, 0); // MH: don't clobber voice channel
					}
...
//		points = damage - 0.5 * VectorLength (v);
		points = damage * (1.0 - VectorLength (v)/radius);		// Ridah, changed this (MH: re-enabled)

//		points = (damage * (1.0 - VectorLength (v)/radius)) * 0.5;//FREDZ (MH: disabled)


//		points = (damage * 0.5) * (1.0 - VectorLength (v)/radius); //FREDZ
...
//						if (deathmatch->value)
						{	// play the sound on different channels so it's less likely to get over-written
							if (ent->last_talk_time == level.time && ent->last_voice && ent->last_voice->soundindex) //hypov8 bugfix: null on cast
							{
	...
	while ((ent = findradius(ent, inflictor->s.origin, radius)) != NULL)
	{
		...
		if (points > 0)
		{
			if (CanDamage (ent, inflictor))
			{
				...
				{
					T_Damage (ent, inflictor, attacker, vec3_origin, ent->s.origin, vec3_origin, points, points, 0, MOD_FLAMETHROWER); // MH: changed from MOD_FLAMETHROWER
				}
			}
		}
	}
