	...
	case PAWN_WEAPONS: // gun item
		...
		else if (pawn_item_index == 3)
		{
			...
			if (item)
			{
				...
				if (ent->client->pers.inventory[index] > ent->client->pers.max_308cal)
					ent->client->pers.inventory[index] = ent->client->pers.max_308cal;
		...
		else if (pawn_item_index == 6)
		{
			...
			if (item)
			{
				...
				if (ent->client->pers.inventory[index] > ent->client->pers.max_gas)
					ent->client->pers.inventory[index] = ent->client->pers.max_gas;
...
qboolean PawnItemPriceSay (int item)
{
	...
	if (current_pawn_item == PAWN_MODS)
	{
		if (pawn_item_index <= 3) // player wants a mod for pistol
		{
			...
			else if (pawn_item_index == 1 && player->client->pers.inventory[ITEM_INDEX(FindItem("Pistol RoF Mod"))]  )
			{
				PawnSay (pawnomatic_specific, (pawnomatic_specific[15].last_played < pawnomatic_specific[14].last_played) + 14);
				demo_hack = false;
			}
			else if (pawn_item_index == 2 && player->client->pers.inventory[ITEM_INDEX(FindItem("Pistol Magnum Mod"))] )
			{
				PawnSay (pawnomatic_specific, (pawnomatic_specific[15].last_played < pawnomatic_specific[14].last_played) + 14);
				demo_hack = false;
			}
			else if (pawn_item_index == 3 && player->client->pers.inventory[ITEM_INDEX(FindItem("Pistol Reload Mod"))])
			{
				PawnSay (pawnomatic_specific, (pawnomatic_specific[15].last_played < pawnomatic_specific[14].last_played) + 14);
				demo_hack = false;
			}
		...
		else if (pawn_item_index == 4) // player wants to buy the cooling jacket for hmg
		{
			...
			else if (player->client->pers.inventory[ITEM_INDEX(FindItem("HMG Cooling Mod"))])
			{
		...
		if (current_pawn_item == PAWN_AMMO1 || current_pawn_item == PAWN_AMMO2)
		{
			...
			case 1:
				...
				else
				{
					...
					{
						idx = ITEM_INDEX(it);
						if (player->client->pers.inventory[idx] >= player->client->pers.max_gas)
						{
							PawnSay (pawnomatic_specific, 16);
							return false;
						}
			...
			case 2:
					...
					{
						idx = ITEM_INDEX(it);
						if (player->client->pers.inventory[idx] >= player->client->pers.max_308cal)
						{
							PawnSay (pawnomatic_specific, 16);
							return false;
						}
